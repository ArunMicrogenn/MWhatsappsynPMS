...
import { Activity, Play, Square, RotateCw, Terminal, CheckCircle2, AlertTriangle, Cpu, HardDrive, RefreshCcw, Wifi, Send, Code, ArrowRight, Clock, FileText, Check, AlertCircle, Copy, Radio, Sparkles, CheckCheck, ShieldCheck, Zap } from 'lucide-react';
...
export function ServiceSimulator({ serviceName, config, darkMode }: ServiceSimulatorProps) {
  const [status, setStatus] = useState<'Running' | 'Stopped' | 'Starting' | 'Stopping'>('Running');
  const [activeTab, setActiveTab] = useState<'monitoring' | 'benchmarking' | 'webhook'>('monitoring');
